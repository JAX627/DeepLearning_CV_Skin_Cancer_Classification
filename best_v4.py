...
sample_weights = [0] * len(img_ds['train'])
for idx, (img_path, label) in enumerate(img_ds['train'].imgs):
    class_weight = class_weights[label]
    sample_weights[idx] = class_weight
sampler = WeightedRandomSampler(sample_weights, num_samples=len(sample_weights), replacement=True)


dl = {'train': DataLoader(img_ds['train'], batch_size=32, sampler=sampler),
    'val': DataLoader(img_ds['val'], batch_size=32, shuffle=True)}
